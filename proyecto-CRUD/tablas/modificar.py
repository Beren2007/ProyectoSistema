...
def crear_modificar(root,tree):
    ...
    def obtener_valores(var1,var2,var3,var4,var5,var6):
        ...
        vare6=var6.get()
        if len(str(vare1)) != 2:
            messagebox.showinfo("Aviso","logitud del curso debe ser de 2 caracteres")
        elif  len(str(vare1)) == 2:
            patron = re.compile(r'^[a-dA-D]+$')
            van1=int(vare1[0:1])                  
            if van1 in range(1,4) and not patron.match(vare1[1:2]):
                messagebox.showinfo("Aviso","La sección debe ser entre A y la D")
            elif van1 in range(4,8) and int(vare1[1:2]) not in range(1,7):
                messagebox.showinfo("Aviso","La sección debe ser el 1 al 6")
            elif vare6=="":
                ...
                conexion.close() 
                top.destroy()
                messagebox.showinfo("Aviso","Actualizacion realizada")    
